...
public class FastCollinearPoints {
    ...
    public FastCollinearPoints(Point[] points) {
        ...
        for (int i = 0; i < N; i++) {
            Point p = clonedPoints[i];
            Point[] pointsBySlope = clonedPoints.clone();
            Arrays.sort(pointsBySlope, p.slopeOrder());

            int s = 1;
            while (s < N) {
                LinkedList<Point> candidates = new LinkedList<>();
                final double SLOPE_REF = p.slopeTo(pointsBySlope[s]);

                do {
                    candidates.add(pointsBySlope[s++]);
                } while (s < N && p.slopeTo(pointsBySlope[s]) == SLOPE_REF);
